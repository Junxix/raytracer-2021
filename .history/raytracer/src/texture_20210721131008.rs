...
impl ImageTexture {
    ...
    pub fn new(filename: &str) -> Self {
        ...
        let data2 = image::open(filename).unwrap().to_rgb();
        let width2 = data2.width();
        let height2 = data2.height();

        let bytes_per_scanline = bytes_per_pixel * (width2.clone() as i64);
        Self {
            bytes_per_pixel,
            data: data2,
            width: width2.clone() as i64,
            height: height2.clone() as i64,
            bytes_per_scanline,
        }
    }
}
